...
const CartContextProvider = function ({ children }) {
  ...
  useEffect(() => {
    if (didMount.current) updateLocalStorage();
    else didMount.current = true;
  }, [stringifiedMinCart]);

  useEffect(() => {
    // PUXA CARRINHO DO LOCALSTORAGE INICIALMENTE E SETA MINIFICADO
  ...
  useEffect(() => {
    // TODA VEZ QUE MINIFICADO MUDA OU TIPO DE USUÁRIO MUDA, PUXA CARRINHO ESTRUTURADO DO BACK
    async function grabCartFromBack() {
      ...
      try {
        let newCart = await api.post(
          'cart',
          minCart,
          accessToken ? config : undefined,
        );
  ...
  useEffect(() => {
    // RESPONSÁVEL PELO COMPORTAMENTO DO POPOVER
    if (lastAddedProduct) {
      setTimeout(() => {
        setLastAddedProduct();
      }, 7000);
  ...
  function addItem(
    ...
    for (let i = 0; i < products.length; i++) {
      if (
        ...
        && subproduct_id
        && subproduct_id === products[i].subproduct_id
      ) {
        id_found = true;
  ...
  function deleteItem(product_id, subproduct_id) {
    if (minCart) {
      const storageProducts = minCart;
      const products = storageProducts.filter(
        (product) => product.product_id !== product_id
          && (!subproduct_id || product.subproduct_id !== subproduct_id),
      );
      productsChanged.current = true;
      setMinCart(products);
      setUpdate(!update);
  ...
  function changeQuantity(product_id, subproduct_id, quantity) {
    ...
    const index = cart.findIndex(
      (element) => element.product_id === product_id
        && (!element.subproduct_id || element.subproduct_id === subproduct_id),
    );
    if (quantity > 0) {
