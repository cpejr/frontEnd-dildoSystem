...
export async function callPaymentAPI(
  ...
  const items = products.map((p) =>
    // MONTA ARRAY DE PRODUTOS NO FORMATO DESEJADO
    ({
      Name: p.name,
      Description:
  ...
  shippingOptions.forEach((sh) => {
    // MONTA ARRAY DE FORMAS DE ENVIO NO FORMATO DESEJADO
    if (!sh.Error) {
  ...
  user = await api.get(`/user/${buyer.id}`, {
  ...
  orderID = await api.get('initializeOrder', {
    headers: { authorization: `Bearer ${buyer.accessToken}` },
  });
