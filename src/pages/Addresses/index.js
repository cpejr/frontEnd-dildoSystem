...
function Addresses() {
  ...
  useEffect(() => {
    try {
      ...
      api.get(`/useraddress/${loginContext.id}`, config).then((response) => {
  ...
  async function goToCheckout(address) {
    ...
    try {
      ...
      notification.open({
        message: "Erro!",
        description: "Houve um erro ao tentar criar o seu pedido.",
  ...
  async function handleSubmitExistingAddress() {
    if (selected >= 0) {
      ...
      notification.open({
        message: "Erro!",
        description: "Selecione um dos seus endereços cadastrados.",
        className: "ant-notification",
        top: "100px",
        icon: <AiOutlineCloseCircle style={{ color: "#F9CE56" }} />,
        style: {
  ...
  async function handleSubmitNewAddress(e) {
    ...
    if (
      ...
      try {
        ...
        notification.open({
          message: "Erro!",
          description: "Ocorreu um erro no cadastro desse endereço.",
          className: "ant-notification",
          top: "100px",
          icon: <AiOutlineCloseCircle style={{ color: "#F9CE56" }} />,
          style: {
      ...
      notification.open({
        message: "Erro!",
        description: "Preencha todos os campos para enviar um novo endereço.",
        className: "ant-notification",
        top: "100px",
        icon: <AiOutlineCloseCircle style={{ color: "#F9CE56" }} />,
        style: {
      ...
      <Spin size="large" spinning={spinning}>
        <div className="main-addresses-wrapper">
          <div className="addresses-content">
                ...
                {addressList.map((address, index) => (
                  <div className="radio-group-wrapper">
                    ...
                    <FiEdit2
                      onClick={(e) => {
                        e.stopPropagation();
                        handleClickAddressEditButton(address);
                      }}
                    />
                  </div>
                ))}
            ...
            <button onClick={handleSubmitExistingAddress}>
              ...
              <div className="new-address">
                <form onSubmit={handleSubmitEditAddress}>
                  ...
                  <Input
                    ...
                    step="1"
                    onChange={(e) =>
                      setEditAddress({ ...editAddress, number: e.target.value })
                    }
        ...
        <p>
          {address.complement
            ? `${address.street} ${address.number}, ${address.neighborhood}, ${
                address.complement
              } - ${address.city}, ${address.state} - CEP ${formatarCEP(
                address.zipcode
              )}`
            : `${address.street} ${address.number}, ${address.neighborhood} - ${
                address.city
              }, ${address.state} - CEP ${formatarCEP(address.zipcode)}`}
        </p>
      </Radio>
    </div>
  );
